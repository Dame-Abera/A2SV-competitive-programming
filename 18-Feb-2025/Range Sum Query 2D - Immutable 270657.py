# Problem: Range Sum Query 2D - Immutable - https://leetcode.com/problems/range-sum-query-2d-immutable/

class NumMatrix:

    def __init__(self, matrix: List[List[int]]):
        self.prefix=[[0 for _ in range(len(matrix[0])+1)] for _  in range(len(matrix)+1) ]
        
        print(self.prefix)
        for row in range(len(matrix)):
            for col   in range(len(matrix[0])):
                self.prefix[row+1][col+1]=matrix[row][col]-self.prefix[row][col]+self.prefix[row][col+1]+self.prefix[row+1][col]
              
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        print(row2,col2,row1,col1,len(self.prefix[0]),len(self.prefix))
        return self.prefix[row2+1][col2+1]-self.prefix[row2+1][col1]-self.prefix[row1][col2+1]+self.prefix[row1][col1]
       


# Your NumMatrix object will be instantiated and called as such:
# obj = NumMatrix(matrix)
# param_1 = obj.sumRegion(row1,col1,row2,col2)